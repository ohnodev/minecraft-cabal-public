...
4) Minecraft runtime files
   ...
   - Ensure /root/minecraft-cabal/server/ops.json exists from env-specific values (do not commit runtime ops.json):
     ...
     - python3 - <<'PY'
       ...
       PY
   - Ensure Fabric runtime artifacts exist in `/root/minecraft-cabal/server`, with source-of-truth listed:
     ...
     - `server.properties` (runtime-generated from `server.properties.template` by `scripts/start.sh`)
   ...
   - Verify required runtime artifacts are present before first boot:
...
- These are expected runtime files under `server/` after setup:
  - `fabric-server-launch.jar`, `server.jar`, `libraries/`, `.fabric/`, `versions/`, `eula.txt`, `server.properties`
