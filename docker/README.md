...
- Under **`server/`**: the full Fabric runtime tree — **`fabric-server-launch.jar`**, **`server.jar`**, **`libraries/`**, **`versions/`**, **`.fabric/`** (from the Fabric installer), plus your **`mods/`** and **`config/`** layout — same as a normal host install. Have **`versions/`** present alongside `libraries/` and the jars before first boot, or the launcher cannot resolve the game version. **[Chunky](https://modrinth.com/mod/chunky)** (Fabric) ships in **`server/mods/`** for optional pre-generation (`/chunky` in-game).
